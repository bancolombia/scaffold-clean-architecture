...
Fork the project [on GitHub](https://github.com/bancolombia/scaffold-clean-architecture) and clone your fork locally.

```bash
git clone git@github.com:username/scaffold-clean-architecture.git
...
We have a pre-commit hook to format the code and ensure the unit tests pass, you only need to install this with the following command.
...
:::info
If you are interested in formatting the code at any time, execute this command:
...
You could follow this [guide](#implementing-or-changing-a-module).
...
* Include tests
* For new features, add the documentation in its respective section within the `/docs` folder.
...
:::danger Required for structural changes
From version 2.3.0 you should implement an UpgradeAction, this implementation will be **required** if you had changed any structural file. If your pull request have structural changes and it does not comes with an UpgradeAction, **it will be rejected**.
:::

This implementation will be required if you had changed any structural file like:
...
The UpgradeAction should be implemented in the package `co.com.bancolombia.factory.upgrades.actions` and should have a specific date-based name, which allows that the update occurs in the order in which it is created.

**Naming convention:** `UpgradeY{year}M{month}D{day}{DescriptiveName}`

Where:
- `Y{year}` = Year (e.g., Y2024)
- `M{month}` = Month (e.g., M01 for January)
- `D{day}` = Day of month (e.g., D19)
- `{DescriptiveName}` = Brief description in PascalCase

**Example:** `UpgradeY2024M01D19SonarSkipCompile`
...
#### Task Parameter Naming Convention

When defining parameters for tasks (e.g., `gradle gep --type webflux`), follow these naming standards:

- **Multi-word parameters**: Separate words with hyphens
  - Example: `--server-name`, `--module-type`, `--base-package`
- **Single-word parameters**: Use without hyphens
  - Example: `--type`, `--name`, `--package`

:::tip
Consistent parameter naming improves usability and maintains a uniform CLI experience across all tasks.
:::
...
As you can research by using the project, it generates a multi-module Gradle project, each module represents a subproject that has some considerations:
...
All our modules are generated based on templating using Mustache, these templates are located in the resources directory of the project.

Actually these templates should consider some variables that will impact the template, these variables are:
...
- All constants defined in [`Constants`](https://github.com/bancolombia/scaffold-clean-architecture/blob/master/src/main/java/co/com/bancolombia/Constants.java) with the same constant name, for example:
...
#### 1. Add the resource templates

Add the resource templates in the correct location depending of your module type and the project type (reactive or not reactive).
...
#### 2. Implement the ModuleFactory

Implement the ModuleFactory for the new module type in the correct package.
...
This method gets or creates a new ObjectNode of the application.yaml file, and you can add the custom properties to this file. For example:

The next code adds the property `spring.datasource.url=jdbc:h2:mem:test` and the property `spring.datasource.driverClassName=org.h2.Driver`
...
#### 4. Add the instantiation block

Add the instantiation block in the correct class according to the new type.
...
And then add the value to the params

```java
builder.addParam("task-param-name", name);
```

Then use the param in code or template. In code:
...
Make the changes which you want to try.

### 2. Change the version

Change the version of the plugin for your custom own version in the `gradle.properties` file, for example:
...
Publish the plugin to mavenLocal by running:
...
Generate a new clean architecture project using the local plugin.

To achieve this you have two options:

#### Option A: Using build.gradle
...
#### Option B: Using settings.gradle
