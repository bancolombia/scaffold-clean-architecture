...
public class GenerateEntryPointTask extends DefaultTask {
    ...
    private void generateApiRest(String packageName) throws Exception  {
        ...
        getProject().file(entryPointDir.concat("/").concat(Constants.MAIN_JAVA).concat("/").concat(packageName).concat("/").concat(entryPoint).concat("/").concat(Constants.API_REST_CLASS).concat(Constants.JAVA_EXTENSION));
