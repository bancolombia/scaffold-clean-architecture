...
public class GenerateDrivenAdapterTask extends DefaultTask {
    ...
    private DrivenAdapterFactory drivenAdapterFactory = new DrivenAdapterFactoryImpl();
    ...
    private void throwDrivenAdapterTask(){
        if (numberDrivenAdapter < 0) { throw new IllegalArgumentException("No Driven Adapter is set, usege: gradle generateDrivenAdapter --value numberDrivenAdapter"); }
    }
    ...
    private void writedFiles() throws IOException {
        Utils.writeString(getProject(), drivenAdapter.getDrivenAdapterDir().concat("/").concat(Constants.BUILD_GRADLE), drivenAdapter.getBuildGradleContentEntryPoint());
