...
public class ApiRest {
	...
	public Mono<ResponseEntity<Authentication>> saveAuth(@RequestBody Authentication authentication) {
		return authUC.saveAuthentication(authentication)
				.map(r -> ResponseEntity.ok().body(r))
				.onErrorResume(Mono::error);
	}

	@GetMapping(path = "/auth/{username}/{channelId}")
	public Mono<ResponseEntity<Authentication>> findAuthentication(@PathVariable String username, @PathVariable String channelId) {
		return authUC.findByUsernameAndChannelId(username, channelId)
	...
	public Mono<ResponseEntity> deleteAuthentication(@PathVariable int id) {
		return authUC.deleteAuthentication(Authentication.builder().id(id).build())
